...
- Can get biased estimates of variance when number of groups is small
...
Missing data is common in longitudinal data. Before starting the analysis, it is important to understand the missing data patterns. Use the `skim` function from the **skimr** R package to get a quick view of the missigness.
...
- **Last observation carried forward**: Keep the last observation from each group (school in this case) and conduct analysis for independent observations. 
...
- **Apply multilevel methods**: Estimate patterns using available data recognizing that trends for groups with complete data re more precise than for those with fewer measurements. This is under the condition that the probability of missingness does not depend on unobserved predictors or the response.
...
.center[
.question[
What is an advantage of each method? What is an disadvantage?
]
]
...
**Level One (within school)**: How did average math scores within a school change over time? 
...
Start with the **unconditional means model** a model with no covariates at any level

- Also called the random intercepts model
...
$$\begin{aligned}&Y_{ij} = \alpha_0 + \beta_0Year08_Pij + u_i + v_iYear08_{ij} + \epsilon_{ij} \\[10pt]
&\epsilon_{ij} \sim N(0, \sigma^2) \hspace{1mm} \text{  and  } \hspace{1mm}\left[ \begin{array}{c}
            u_{i} \\ v_{i}
...
$$Pseudo R^2 = \frac{\hat{\sigma}_.(Model 1) - \hat{\sigma}_.(Model 2)}{\frac{\hat{\sigma}_.(Model 1)}}$$
...
Model in section 9.6.3

- Write it out
- Notice difference in parameters use for sloeps and intercepts - what does this mean 

- Interpret the coeffcients for the model 
