...
p1  / p2 + plot_layout(heights = c(20,20))
...
ggplot() +   
  ...
  labs(title = "Observed (gray) vs. Expected (blue) number of drinks")
```
...
We introduce a new parameter $\alpha$ for the proportion of two zeros, then fit a model that has two components
...
`r emo::ji("one")` Association, among drinkers, between the mean number of drinks and predictors sex and off campus residence
...
.pull-right[
.center[**How they work**]

- Remove the expected Poisson distribution (blue) from the observed (gray). Some zeros will remain; these are the true zeros (non-drinkers). A reasonable estimate for $\alpha$ is the proportion of such zeros 

- Use the likelihood function and iterative process using <u>all</u> data to fit the models. 
