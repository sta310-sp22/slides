...
`r emo::ji("three")`  Find the difference in the log likelihoods

`r emo::ji("four")`  Use inference to determine if the difference is statistically significant 
...
$$\begin{aligned}\max\{&\log(Lik(p_{H|N},p_{H|H Bias},p_{H|V Bias}))\}\\
&= -62.61375\end{aligned}$$
]
...
$$\begin{aligned}\max\{&\log(Lik(p_{H|N},p_{H|H Bias},p_{H|V Bias}))\} - \max\{\log(Lik(p_H)\}\\ &= -62.61375 - (-62.36102) = \textbf{-0.25273}\end{aligned}$$
...
.center[
**Is the difference in the max (log) likelihoods statistically significant?** 
]
---
...
- How we'll use likelihoods in this course

- These examples don't include covariates! 
