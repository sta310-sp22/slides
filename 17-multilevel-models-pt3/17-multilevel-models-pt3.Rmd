...
- Fit and interpret multilevel model 

- Compare maximum likelihood (ML) and restricted maximum likelihood (REML) estimation approaches

- Understand general process for fitting and comparing multilevel modelss
...
- The **error terms** are $u_i, v_i, \epsilon_{ij}$
  - Describe the behavior of the random effects

--

Note that we no longer need to estimate $a_i$ and $b_i$ (the random effects) directly as we did earlier. 

---

## Notation 

- Greek letters denote the final fixed effect model parameters that will be estimated
  - e.g., $\alpha_0, \alpha_1, \beta_0, \beta_1$
  
- Roman letters denote the preliminary effects at lower levels that will not be estimated directly
  - e.g. $a_i, b_i$
...
- **Level One**: Errors associated with each performance of a given musician are are normally distributed  e.g. error associated with each performance of a given musician is $\epsilon_{ij} \sim N(0, \sigma^2)$
...
$\sigma_{uv} =  \rho_{uv}\sigma_{u}\sigma_v$
  
- covariance between $u_i$ and $v_i$
- How the error terms vary together
...
[SET UP ACTIVITY - split into groups + each group has a google slide]
...
`Independence (OLS)` <-c("15.72(0.36)", "1.79(0.55)", "-0.28(0.79)", "-1.71(1.06)")
...
table3chp8 <- data.frame(Variable, `Independence (OLS)`, `Two-Stage`, 
                         LVCF, Multilevel)
...
- Accounts for uncertainty in fixed effects estimates, so estimates of variance components are unbiased
...
- **ML** (`REML = FALSE`) <u>must</u> be used if you want to compare nested fixed effects models using a likelihood ratio test (e.g., a drop-in-deviance test).
  - For REML, the goodness-of-fit and likelihood ratio tests can only be used to draw conclusions about variance components the test to draw conclusions about random effects only

.footnote[.small[Source: Singer, J. D., Willett, J. B., & Willett, J. B. (2003). *Applied longitudinal data analysis: Modeling change and event occurrence*. Oxford university press.]]
---
