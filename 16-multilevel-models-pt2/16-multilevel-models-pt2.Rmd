...
## [Click here for PDF of slides](15-multilevel-models-pt2.pdf)
...
p1 <- ggplot(data = music, aes(x = na)) + 
  ...
  labs(x = "Individual negative affect", 
       title = "Individual negative affect scores")
...
<font color = "red">ADD GOOGLE POLL?</font>
---
...
`r emo::ji("two")`  Then fit a system of models to predict the fitted coefficients in the Level One models based on instrument type (Level Two model).
...
.question[
<font color = "red"> Number of models at each stage. 
]
...
.alert[
Read the code in Part 2: Level One models. 

<font color = "red">ADD GOOGLE FORM. What happens if a student only played large_ensembles? what is their estimate for the slope? What is their estimate for the intercept?</font>
...
.question[
<font color = "red">How many observations used to estimate a? b? Make Google form</font>
]
...
- What does it mean for $\sigma_{uv} > 0$? 
- What does it mean for $\sigma_{uv} < 0$? 
